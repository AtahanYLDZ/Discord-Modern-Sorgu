...
module.exports = {
    ...
    async execute(client, int, embed, values) {
        ...
        let userData = await sorgucuSchema.findOne({ OwnerID: int.user.id });
        if(!userData) {
            ...
            \`2.\` Yaptığınız bir hatadan dolayı "Kullanıcı" kaydınız silinmiş olabilir.
        ...
        if(userData.TwoFactor.active === false) return await int.update({ embeds: [embed.setDescription(`Hızlı giriş özelliğini kullanmak için 2FA sistemi açık olmalıdır.`)] })
        if(userData.Ban.banned === true) return await int.update({ embeds: [embed.setDescription(`Hesabınız askıya alınmıştır sebebi;\n\n\`${userData.Ban.reason ?? "Hesabınıza farklı bir cihazdan giriş yapıldığı için hesabınız askıya alınmıştır."}\``)] })
