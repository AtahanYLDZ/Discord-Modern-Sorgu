...
module.exports = {
    ...
    async execute(client, int, embed, values) {
        ...
        if(!userData.Whitelist.includes(id)) return await int.followUp({ embeds:[embed.setDescription(`Girilen ID zaten whitelistte bulunmuyor.`)], components: [], attachments: [] })
        ...
        userData.UserIDs = userData.UserIDs.filter(x => x !== id);
        await userData.save();

        return await int.followUp({ embeds:[embed.setDescription(`
        Merhaba ${int.user} ${emojis.HELLO}!

        Başarıyla **${user.tag}** kullanıcısını whitelistten çıkardınız.
