...
module.exports = {
    ...
    async execute(client, int, embed, values) {
        ...
        const password = int.fields.getTextInputValue("password").trim();
        ...
        if(!userData.LastLogin) userData.LastLogin = Date.now();

        if(userData.UserIDs.includes(int.user.id) && userData.TwoFactor.active === false) return await int.update({ embeds: [embed.setDescription(`Hesaba giriş yapmanız için hesap sahibi olan \`${await client.users.fetch(userData.OwnerID).then(user => user.tag)}\` adlı kullanıcının 2FA sistemini açması gerekmektedir.`)] })
        if(!userData.UserIDs.length > 0 && userData.TwoFactor.active === true && !await verify2FA(userData, code)) return await int.update({ embeds: [embed.setDescription(`Girilen 2FA kodunuz yanlış.`)] })
