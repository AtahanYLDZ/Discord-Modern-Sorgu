...
module.exports = {
    ...
    async execute(client, int, embed, values) {
        ...
        .setDescription(`${slicedVeri.map((x, i) => `TC: \`${x.TC}\`\nADI & SOYADI: \`${x.ADI} - ${x.SOYADI}\`\nKATILIM ÜCRETİ: \`${x.KATILIMUCRETI}\`\nREÇETE NO: \`${x.RECETENO}\`\nTAHSİS EDİLDİ Mİ: \`${x.TAHSISEDILDIMI}\`\nTAKİP NO: \`${x.TAKIPNO}\`\nTAKİP TARİHİ: \`${x.TAKIPTARIHI}\`\nTESİS ADI: \`${x.TESISADI}\`\nKLİNİK ADI: \`${x.KLINIKADI}\``)}`)
